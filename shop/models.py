...
class Panier(models.Model):
    ...
    total = models.IntegerField(default=0)  # Total en CFA (entier)

    def __str__(self):
        return f"Panier de {self.utilisateur.username}"

    def total_panier(self):
        # Calcule la somme des sous-totaux de chaque ligne du panier
...
class LignePanier(models.Model):
    ...
    @property
    def detail_prix(self):
        """
        Retourne une chaîne détaillant le prix unitaire et le sous-total.
        """
        # Formate le prix unitaire et le sous-total (ici, on affiche le Decimal tel quel ; vous pouvez le formater selon vos besoins)
        return f"Prix unitaire: {self.produit.prix} CFA, Sous-total: {self.sous_total()} CFA"
